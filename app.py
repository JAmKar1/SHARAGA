...
@login_required
def set_curator():
    """Настройка статуса куратора для преподавателя"""
    user_data = get_user_by_id(session['user_id'])
        ...
        try:
            if action == 'set':
                if group_name:
                    ...
                    cursor.execute('SELECT COUNT(*) FROM users WHERE group_name = ?', (group_name,))
                    if cursor.fetchone()[0] > 0:
...
if __name__ == '__main__':
    ...
    if check_and_fix_db():
        ...
        print("✅ Система кураторства для преподавателей готова")
        ...
        print("👨‍🏫 Преподаватели могут стать кураторами групп")
